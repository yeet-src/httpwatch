 ...
 * @param {(part:object)=>void} [opts.onPart]    called with each body-part line
 * @param {(evt:object)=>void} [opts.onStatus]   lifecycle notifications
 * @returns {Promise<{stop:()=>Promise<void>}>}
 */
export async function startExporter(opts) {
  const { yeetBin, agentDir, socket, userSocket, scriptArgs = [], onSnapshot, onPart = () => {}, onStatus = () => {} } = opts;
  let stopped = false;
  let currentWs = null;
  let currentId = null;
  ...
  async function spawnOnce() {
    ...
    ws.onmessage = (ev) => {
      ...
      while ((nl = lineBuf.indexOf("\n")) >= 0) {
        ...
        // Heads and bodies come as their own lines ahead of the snapshot that
        // references them — one line per body part, so a big body is bounded by
        // the per-line ceiling rather than by what a snapshot has room for.
        else if (msg.t === "part") onPart(msg);
