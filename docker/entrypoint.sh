...
# 0b. Persisted state (alert rules). When /data is a bind mount that Docker had
#     to create, it lands root-owned — so the file the server writes there would
#     be unreadable-in-practice for whoever started the container. STATE_UID/GID
#     (passed by `make` and by the documented `docker run`) hands it back. Purely
#     cosmetic for a named volume, so failures are ignored.
STATE_DIR="$(dirname "${ALERTS_FILE:-/data/alerts.json}")"
if [ -n "${STATE_UID:-}" ] && [ -d "$STATE_DIR" ]; then
  chown "${STATE_UID}:${STATE_GID:-$STATE_UID}" "$STATE_DIR" 2>/dev/null \
    && echo "[entrypoint] ${STATE_DIR} owned by ${STATE_UID}:${STATE_GID:-$STATE_UID}" \
    || echo "[entrypoint] could not chown ${STATE_DIR} (harmless for a docker volume)"
fi
